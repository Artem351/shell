...
int parseline(char *line)
{
	...
	switch(*s) {
		...
		default:
	#ifdef DEBUG_PARSE
		fprintf(stderr,"default:*s=%d;nargs=%d\n",*s,nargs);
	#endif
			/* a command argument */
			if (nargs == 0) /* next command */
				rval =ncmds+1;
	#ifdef DEBUG_PARSE
		fprintf(stderr,"default_before,*s=%c;%d\n",*s,*s);
	#endif
			cmds[ncmds].cmdargs[nargs++]= s;
			cmds[ncmds].cmdargs[nargs] = (char *) NULL;
			s = strpbrk(s, delim);	
	#ifdef DEBUG_PARSE
		fprintf(stderr,"default_after,*s=%c;%d\n",*s,*s);
	#endif
			if (isspace(*s))
			#ifdef DEBUG_PARSE
		fprintf(stderr,"debug isspace,*s=%c;%d\n",*s,*s);
	#endif
				*s++='\0';
	#ifdef DEBUG_PARSE
		fprintf(stderr,"default_end\n");
	#endif
			break;
		} /* close switch */
	} /* close while */
